...
Phase 0 (foundation), Phase 1 (core data layer + vertical slice), and Phase 2 (homeowner/detailed layout) are complete and merged to `main`. Phase 3 (interactive controls) is split into 3A/3B/3C by complexity and manufacturer-verification risk, not built as one change — see `ROADMAP.md`. Phase 3A (control framework + `filter_reset_control`) is in progress on `feature/phase-3`. Check `ROADMAP.md` before starting new work; it tracks what's actually done vs. planned.
