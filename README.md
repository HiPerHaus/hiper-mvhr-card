...
- **Configured, but no shower right now** — a calm full-width banner below the lower Temperature/Airflow/System Status cards and directly above "More controls" reads "Shower detection ready."
...
- **Shower detected** (`shower_detected` is `on`) — the full banner changes to an active purple state with a lightweight inline-SVG shower illustration, "Boost active"/"Boost not active" (from the same `boost_active` role the header and status card use), the pipe temperature, the stored trigger temperature, optional boost remaining, and a **re-arm temperature** the card computes itself as trigger temperature − 10°C (never a separate entity) — matching the shower detector's own rearm rule in the [`ha-altair-mvhr`](https://github.com/HiPerHaus/ha-altair-mvhr) integration. Any one of the three sensors being unavailable simply omits that fact; the banner never shows a placeholder or synthesized reading.
