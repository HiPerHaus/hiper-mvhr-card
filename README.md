...
**Shower detection.** Entirely optional, and driven by the same entity-role/availability model as everything else in the card — map none, some, or all of `shower_detected` / `shower_trigger_temperature` / `shower_pipe_temperature` plus the editable `shower_temperature_rise` and `shower_detection_window` number roles, and the card adapts:

- **Not configured at all** — nothing shower-related renders anywhere.
- **Configured, but no shower right now** — a calm full-width banner below the lower Temperature/Airflow/System Status cards and directly above "More controls" reads "Shower detection ready." If the two number entities are mapped, the same banner includes native editable controls for the temperature rise and detection window.
...
- **Shower detected** (`shower_detected` is `on`) — the full banner changes to an active purple state with a lightweight inline-SVG shower illustration, "Boost active"/"Boost not active" (from the same `boost_active` role the header and status card use), the pipe temperature, the stored trigger temperature, optional boost remaining, editable detection settings when mapped, and a **re-arm temperature** the card computes itself as trigger temperature − 10°C (never a separate entity) — matching the shower detector's own rearm rule in the [`ha-altair-mvhr`](https://github.com/HiPerHaus/ha-altair-mvhr) integration. Any unavailable/missing setting or sensor is handled independently: missing controls are hidden, unavailable controls are safely disabled, and the banner never shows a fake sensor reading.
